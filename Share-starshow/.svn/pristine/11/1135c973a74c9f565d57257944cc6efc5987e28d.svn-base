...
var photos = {
	init:function() {
		...
		// this.title     = this.imagesInfo.getElementsByClassName("titles").eq(0),
		// this.brand       = this.imagesInfo.getElementsByClassName("brand").eq(0),
	...
	getClient:function(e) {
		...
		if(!common.$('picBox')) {
			...
			photos.picBox.style.cssText = "width:"+photos.clientW+"px;height:"+this.clientH+"px;background:url(../public/images/tbg.jpg) no-repeat;overflow:hidden;";
	...
	showImgData:function(data) {
		var _this = this;
		this.json = data;
		if(data.show[0].pictures[0].url) {
			this.pictureurls = data.show[0].pictures;
			this.titles=data.show[0].title;
			this.brands=data.show[0].brand;
			if(data.show[0].photo!=undefined)
			{
				this.laiy=data.show[0].photo;
			}else
			{
				this.laiy="";
			}
		...
		var imgs  = [];
		this.thumb = [];
		for(var i=0,l=this.pictureurls.length;i<l;i++) {
			var p = this.pictureurls[i];
			imgs.push('<li class="picsContent" style="width:'+this.clientW+'px;line-height:'+this.clientH+'px;text-align:center;"><img class="HQlazy"  src="http://test-trendsapi.bazaarstar.cn/public/share/public/images/loading.gif" original="'+p.url+'" align="absmiddle" /></li>');
		...
		this.picLength = this.json.relation?this.pictureurls.length+1:this.pictureurls.length;
	...
	writeText:function() {
		...
		this.totle.innerHTML =' Look '+ (this.picRoot+1)+' of '+this.pictureurls.length;
		document.getElementsByClassName("titles").eq(0).innerHTML =this.titles;
		document.getElementsByClassName("brand").eq(0).innerHTML =this.brands;
		document.getElementsByClassName("laiy").eq(0).innerHTML =this.laiy;
