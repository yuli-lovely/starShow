...
var photos = {
	init:function() {
		...
		this.headerTitle = this.header.getElementsByClassName("tit").eq(0);
		this.totle       = this.imagesInfo.getElementsByClassName("tot").eq(0),
	...
	showImgData:function(data) {
		var _this = this;
		this.json = data;
		if(data.show.pictures[0].url) {
			this.pictureurls = data.show.pictures;
			this.titles=data.show.title;
	...
	writeText:function() {
		...
		this.totle.innerHTML =(this.picRoot+1)+' / '+this.pictureurls.length;
		document.getElementsByClassName("tit").eq(0).innerHTML =this.titles;
		document.getElementsByClassName("content").eq(0).innerHTML =this.pictureurls[this.picRoot].desc;
